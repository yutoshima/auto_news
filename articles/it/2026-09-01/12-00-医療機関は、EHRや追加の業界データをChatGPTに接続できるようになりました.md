---
title: "医療機関は、EHRや追加の業界データをChatGPTに接続できるようになりました"
source: "OpenAI Blog"
category: "it"
published: 2026-09-01T12:00:00
url: https://openai.com/index/chatgpt-connects-health-records-and-healthcare-sources
---

# 医療機関は、EHRや追加の業界データをChatGPTに接続できるようになりました

## メタデータ

- **情報源**: OpenAI Blog
- **カテゴリ**: it
- **公開日時**: 2026年09月01日 12:00
- **URL**: [https://openai.com/index/chatgpt-connects-health-records-and-healthcare-sources](https://openai.com/index/chatgpt-connects-health-records-and-healthcare-sources)

## 概要

ChatGPTは信頼できる医療データに接続できるようになり、臨床医が患者の文脈情報や医療研究などに安全にアクセスできるよう支援します。

---

*この記事は自動収集システムによって保存されました。*
